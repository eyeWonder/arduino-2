...
long mediumInterval = 2000; // medium interval timer
  ...
  void move() {
   ...
   // if (radioPWM > 1115) { //see if we should move
        ...
        if (radioPWM > 1450) { //We are moving forward
          //Serial.print(radioPWM);
          //Serial.println(" we are moving forward");
          
          int motorSpeed = map(radioPWM, 1450, 1910, 30, 255);
...
//      }
  ...
  void setMotorSpeeds(int motorSpeed) {
    ...
    Serial.print("raw motor speed ");
      ...
      if (turnAmount < 1400) { //we ar turning right
      
        // so we reduce the speed of the right track
        int adjustSpeedBy = map(turnAmount, 1050, 1399, motorSpeed, 0);
      ...
      } else if (turnAmount > 1500) { //we ar turning left
        
        int adjustSpeedBy = map(turnAmount, 1499, 1800, 0, motorSpeed);
